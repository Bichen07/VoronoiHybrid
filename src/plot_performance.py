...
def read_performance_file_two(filename):
    """Reads the performance data from the given file and returns parsed data."""
    num_points = []
    scipy_times = []
    c_fortune_times = []
    ...
    return num_points, scipy_times, c_fortune_times


def plot_performance_two(x, y_scipy, y_fortune, log_scale=False):
    """Plots the performance data, with an optional log scale."""
    plt.figure(figsize=(10, 6))
    ...
    filename = f'{folder}/performance_plot2_{"log" if log_scale else "original"}.png'
    plt.savefig(filename)

if __name__ == '__main__':
    # Read data from the performance file
    filename = 'src/performance.txt'
    ...
    filename2 = 'src/performance_two.txt'
    num_points, scipy_times, c_fortune_times = read_performance_file_two(filename2)
    # Plot the data (original scale and log scale)
    plot_performance_two(num_points, scipy_times, c_fortune_times, log_scale=False)
    plot_performance_two(num_points, scipy_times, c_fortune_times, log_scale=True)
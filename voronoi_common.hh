...
struct arc {
   point p; //The focus of the parabola (site point).
   arc *prev, *next; //Links to neighboring arcs.
   event *e;

   seg *s0, *s1; // Half-edges of the Voronoi diagram connected to this arc, ledt edge s0, right edge s1.
...
struct seg {
   ...
   seg(point p)
      : start(p), end(0,0), done(false) // Start point only at new site event intersection with arc.
